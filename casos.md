...
## 15 · microsoft/agent-governance-toolkit — MSRC caso 136137 (VULN-211868)

**Link**: https://github.com/microsoft/agent-governance-toolkit
**Estado**: **Cerrado por MSRC el 24-ago-2026 como Low severity, sin CVE. El comportamiento fue reconocido y el fix quedó planificado para una release futura.** Reserva de disclosure cumplida; detalle publicado el 6-sep-2026.

Qué planteé: `PolicyRule._eval_expression`, en `agentmesh.governance.policy` — el módulo detrás del quick-start `govern()` del README — parsea las condiciones de las reglas contra seis formas fijas por regex: `==`, `!=`, `in [...]`, comparación numérica, atributo booleano suelto, y partido plano por `and`/`or`. Una condición sintácticamente natural pero fuera de esa gramática (`not`, paréntesis, `contains`, wildcards) cae a un `return False` final: sin excepción, sin log, sin marca de auditoría. `PolicyRule.evaluate()` solo falla cerrado cuando hay una excepción real de Python; una condición no soportada no la produce, así que evalúa como "no matchea". Para una regla `deny`, "no matchea" significa que la regla no dispara nunca, y `PolicyEngine.evaluate()` cae al `default_action` de la política — que el propio ejemplo del README pone en `allow`.

— Reproducción, con el engine sin modificar (commit `7d0cef5` del 12-ago-2026): una política con `default_action: allow` y una única regla `deny` con la condición `"action.type in ['drop', 'delete', 'truncate'] and not action.approved"`. Un `drop` sin aprobación sobre la tabla `users` devuelve `decision.allowed = True`, con razón `"No matching rules, using default"`. La operación destructiva no aprobada pasa, sin ninguna señal de que algo esté mal. El `not` de esa condición es la sintaxis no soportada. `validate_policy_schema()` tampoco lo agarra: verifica que `condition` no esté vacío pero nunca parsea la expresión, así que la política pasa la validación limpia y solo falla — en silencio — en tiempo de evaluación.

— Por qué lo reporté como seguridad y no como bug: el ADR-0013 del propio repo declara que el motor "fails closed on all evaluation errors", que no existen flags para comportamiento fail-open, y lo justifica porque los errores fail-open "are silent and may go undetected". Exactamente el caso que el ADR declara imposible ocurre por control de flujo completamente normal, sin que ninguna excepción se levante jamás. Sugerí severidad High según la tabla del SECURITY.md del propio proyecto (policy bypass bajo condiciones específicas), y como dirección de fix, que `_eval_expression` levante excepción ante una condición que no matchea ninguna forma conocida — con eso el fail-closed existente la atraparía — e idealmente validar la gramática en tiempo de carga. Alcance declarado en el reporte: `agent_os.policies`, el otro modelo de condiciones del mismo repo, no está afectado — es un switch cerrado sobre un enum, sin caso de sintaxis no soportada.

Qué pasó: lo reporté el 14-ago-2026 por el MSRC Researcher Portal (`secure@microsoft.com` rebota los reportes con un redirect automático al portal; dejé asentada esa fricción en el propio reporte). MSRC abrió el caso 136137 el mismo día y pidió confidencialidad durante la investigación. El 24-ago-2026 cerró la revisión: **reconocen el comportamiento descripto** ("we acknowledge the behavior described in your report"), lo evalúan **Low severity** — dicen que no representa un salto de frontera de seguridad —, no emiten CVE, no lo trackean más, y el feedback pasó al equipo de producto con el fix "planned to be addressed in a future release". La ficha del mismo caso, sin embargo, clasifica el impacto como **Security Feature Bypass**.

— Lo que queda expuesto hoy, a la fecha de esta publicación y hasta esa release futura: cualquier despliegue que use `agentmesh.governance.policy` con reglas `deny` escritas fuera de las seis formas soportadas tiene esas reglas inertes sin saberlo — pasan la validación de esquema, no dispara ninguna excepción, y con el `default_action: allow` del ejemplo oficial las operaciones que la regla debía bloquear se permiten todas. El autor de la política no tiene forma de enterarse desde la herramienta: ni al cargar, ni al evaluar, ni en el registro de auditoría.

— La lectura: es el patrón de toda esta línea de trabajo, un nivel más arriba. Un motor de gobernanza cuyo documento de arquitectura promete que los errores de evaluación nunca fallan abiertos, falla abierto en silencio; el vendor lo reconoce, lo va a arreglar, y la corrección va a viajar sin CVE, sin advisory y sin tracking — un fail-open reconocido que se corrige en silencio. Publico el detalle con el caso ya cerrado y la reserva cumplida: si para el vendor no amerita servicing de seguridad, tampoco amerita reserva.
...
**Estado**: **Closed (completed)**, `priority/p1`. Tres reproducciones independientes de terceros. 14 reacciones +1.
...
Qué pasó: el 8-sep, veintiuna horas después del comentario, SandyTao520 abrió la PR #29252, `fix(core): preserve explicit versioned Flash model IDs`, la mergeó a las 22:54:39Z y cerró el issue como completado dos segundos después. Tres archivos, +54/-6: cinco líneas en `packages/core/src/config/models.ts` y cuarenta y nueve de test. Entró la mitad estrecha, la que preserva los ids explícitos. Verificado contra el parche: no hay `throw`, ni `warn`, ni `console`, ni `exit`, ni reconocimiento del operador. Corrigieron el nombre. No pusieron la compuerta.
...
Caso 15 — microsoft/agent-governance-toolkit (MSRC 136137) — **cerrado por MSRC como Low, sin CVE, fix planificado** — comportamiento reconocido; detalle completo publicado tras cumplirse la reserva
...
Caso 25 — knip #1949 — **Closed** — aceptado por el mantenedor y derivado a `#1532`, que al 9-sep sigue abierto
