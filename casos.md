...
## Casos 23-33 · segundo rastrillaje (6 categorías nuevas: secret scanning, type-checker baselines, dead-code baselines, duplicate-code detectors, dependency-vuln suppression, quality gates de código nuevo)

Filed con el mismo registro de sala terse. Metodología: 6 búsquedas en paralelo, 24 herramientas evaluadas, código fuente real leído en cada caso, repro en vivo priorizado sobre code-only. 9 de 10 candidatos con repro en vivo se llegaron a filear — el décimo, **mypy-baseline** (orsinium-labs/mypy-baseline), tiene "Issue creation is restricted in this repository" y no tiene Discussions habilitadas: vulnerable confirmado con repro en vivo (preserve_position=False por default permite swap de mismo mensaje en otra línea del mismo archivo, invisible), pero sin canal público para reportarlo. Queda documentado acá sin issue. Los 2 candidatos code-only (maven-pmd-plugin, OWASP Dependency-Check) se filearon después, con nota de honestidad explícita en el propio issue.
...
| 26 | osprey-oss/deptry (ex fpgmaas/deptry) [#1654](https://github.com/fpgmaas/deptry/issues/1654) | Open | En vivo — `--per-rule-ignores` matchea por nombre de módulo puro (DEP001/003/004) |
| 27 | kucherenko/jscpd [#938](https://github.com/kucherenko/jscpd/issues/938) | Open | En vivo — `ThresholdReporter` solo mira % agregado |
...
| 30 | pypa/pip-audit [#1113](https://github.com/pypa/pip-audit/issues/1113) | Open | En vivo — `--ignore-vuln` global sin scoping de paquete/versión |
...
| 18 | Bachmann1234/diff_cover [#619](https://github.com/Bachmann1234/diff_cover/issues/619) | Open | En vivo — `total_percent_covered()` agrega todo el diff, bug real diluido por líneas triviales |
...
Ningún caso de este bloque tiene respuesta todavía — recién posteados. Próxima verificación de novedades debe incluir estos 7 además de los 15 anteriores.
                                    ...
                                    - - Completé el formulario del MSRC Researcher Portal, incluyendo un párrafo de reclamo explícito sobre la fricción del proceso (el bounce de email) y pedido de un canal de contacto directo para próximas veces, tal como se pidió.
                                      - - **Enviado.** Status: Submitted. Security Impact: Security Feature Bypass. Producto: Copilot, AI + ML, and LLMs. Commit de referencia: `7d0cef5d9820a865c3c19b07bd39ecf7053b58a1`.
                                        - - Pendiente: respuesta de MSRC Case Management, que según el propio portal llega vía la pestaña Activity del reporte, no por email.
                                          ...
                                          | 5 | Hypothesis #4854 | **Closed** | Sí — cerrado por política AI, respondí, sin reapertura |
                                          | 6 | spec-kit #4106 | Open | **Sí — colaboración activa, feedback positivo** |
                                          | 7 | eslint #21223 | Open | No |
                                          | 8 | phpstan #15078 | **Closed (completed)** | Sí — respuesta hostil, respondí, mantenedor volvió con respuesta técnica sustantiva y cerró |
                                          ...
                                          | 12 | golangci-lint #6714 | **Closed (declined)** | Sí — declined, respondí, sin reapertura |
                                          ...
                                          | 18 | diff_cover #619 | Open | No (recién posteado) |
                                          ...
                                          | 26 | deptry #1654 | Open | No (recién posteado) |
                                          | 27 | jscpd #938 | Open | No (recién posteado) |
                                          | 28 | simian #23 | Open | No (recién posteado) |
                                          | 29 | audit-ci #356 | Open | No (recién posteado) |
                                          | 30 | pip-audit #1113 | Open | No (recién posteado) |
                                          | 31 | safety #907 | Open | No (recién posteado) |
                                          | 32 | maven-pmd-plugin #724 | Open | No (recién posteado, code-only) |
                                          | 33 | dependency-check #8751 | Open | No (recién posteado, code-only) |
